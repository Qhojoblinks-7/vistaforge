  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.token) {
        ...
        // Try to fetch current user profile and update AuthContext
        try {
          const me = await apiService.getMe();
          // Populate context with user and token
          login(me, result.token);
        } catch (meError) {
          console.warn('Failed to fetch user profile after login:', meError);
          // Fallback: still set minimal auth state in context
          login({ username: formData.username }, result.token);
        }

        console.log('Exclusive admin login successful, redirecting to dashboard');
    ...
    <main className="bg-white min-h-screen flex items-center justify-center px-4" id="main-content">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-[#0015AA] mb-2">Exclusive Admin Access</h1>
          <p className="text-gray-600">Restricted access for authorized administrator only</p>
        </div>
        ...
        <div className="mt-8 text-center">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
            <div className="flex items-center justify-center mb-2">
              <BsShieldLock className="text-red-500 mr-2" size={16} />
              <span className="text-red-700 font-semibold text-sm">EXCLUSIVE ACCESS</span>
            </div>
            <p className="text-xs text-red-600">
              This admin interface is restricted to a single authorized user only.
              <br />
